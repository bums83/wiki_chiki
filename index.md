...
| [Firecracker]({{ '/wiki/infra/firecracker' | relative_url }}) | Linux/KVM VMM для microVM: API, jailer/seccomp, snapshots и жёсткая граница между VM substrate и control plane |
| [filesql]({{ '/wiki/infra/filesql' | relative_url }}) | Go-библиотека для загрузки data files в in-memory SQLite и SQL-запросов/joins без отдельного database server |
| [Milvus]({{ '/wiki/infra/milvus' | relative_url }}) | Специализированная distributed vector database: dense/sparse ANN, BM25/full-text и hybrid retrieval с отдельными etcd, object storage и WAL слоями |
...
| [Snowball — обучающийся агент скидок FMCG]({{ '/wiki/llm-agents/snowball' | relative_url }}) | Kotlin-прототип: LLM выбирает скидку, а persistent evidence memory и counterfactual evaluation проверяют влияние обучения на следующие решения |
| [Reasoning effort в LLM]({{ '/wiki/llm-agents/reasoning-effort' | relative_url }}) | Практика управления model/mode/effort/context budget: измеримый routing parameter, outcome gates, эскалация только проигравших кейсов и границы reasoning tokens |
...
| [Invidious]({{ '/wiki/tools/invidious' | relative_url }}) | Self-hosted alternative frontend/API для YouTube: Crystal, PostgreSQL, Invidious companion и обратный прокси; доступность зависит от YouTube и operational policy instance |
