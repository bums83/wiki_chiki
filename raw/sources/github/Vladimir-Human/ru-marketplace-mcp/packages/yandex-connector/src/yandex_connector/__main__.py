"""Entry point for the Yandex Market MCP server.

Exposed as the ``yandex-mcp`` console script, so MCP client configs can spawn the
server without knowing where the package lives on disk:

    {"command": "uvx", "args": ["--from", "yandex-connector", "yandex-mcp"]}
...
def main() -> int:
    ...
    from yandex_connector.server import mcp

    return run_server(mcp, server_name="yandex")
